...
import org.jpropelleralt.list.impl.ListSelectionIndices;
...
/**
 * Demonstrate view of {@link Ledger}, with single and multi selection
 */
public class LedgerViewAndMultiSelectionDemo {
	...
	private static void runApp2() {
		...
		Ref<SetBox<Integer>> multiSelection = ListSelectionIndices.create(listBoxRef, true);

		TableView multiTableView = TableView.createMulti(listBoxRef, new Person(), multiSelection, true);
		
		JScrollPane scroll = new JScrollPane(tableView.getComponent());
		
		JScrollPane multiScroll = new JScrollPane(multiTableView.getComponent());
		
		FormLayout layout = new FormLayout(
			//   multilist              list                   editor
				"fill:50dlu:grow, 5dlu, fill:50dlu:grow, 5dlu, pref",
		...
		builder.add(multiScroll,						cc.xy(1, 1));
		builder.add(scroll,								cc.xy(3, 1));
		builder.add(selectionView.getComponent(),		cc.xy(5, 1));
		builder.add(buttons.getComponent(),				cc.xy(3, 3));
		builder.add(selectionIndexView.getComponent(),	cc.xy(1, 3));
