 ...
 */
public class EditablePathReferenceBuilder<R extends Bean, D extends Bean, P extends GenericEditableProp<T>, T> {

	R pathRoot;
	EditablePathPropBuilder<R, D, P, T> builder;
	
	protected EditablePathReferenceBuilder(R pathRoot, EditablePathPropBuilder<R, D, P, T> builder) {
		super();
	...
	/**
	 * Start a {@link EditablePathReferenceBuilder} that can be used to build a {@link PathReference}
	 ...
	 */
	public static <R extends Bean, P extends GenericEditableProp<T>, T> EditablePathReferenceBuilder<R, R, P, T> from(R pathRoot, Class<T> clazz) {
		return new EditablePathReferenceBuilder<R, R, P, T>(pathRoot, EditablePathPropBuilder.<R, P, T>from("model", clazz, pathRoot));
	}
	 ...
	 * 		The path itself
	 */
	public EditablePathReference<T> to(Transformer<? super D, ? extends P> lastTransform) {
		return new EditablePathReference<T>(builder.to(lastTransform));
	}
	 ...
	 * 		The path itself
	 */
	public EditablePathReference<T> to(PropName<P, T> lastName) {
		return new EditablePathReference<T>(builder.to(lastName));
	}
	 ...
	 * 		The path itself
	 */
	public <E extends Bean> EditablePathReferenceBuilder<R, E, P, T> via(Transformer<? super D, ? extends GenericProp<E>> nextTransform) {
		return new EditablePathReferenceBuilder<R, E, P, T>(pathRoot, builder.via(nextTransform));
	}
	/**
	 ...
	 * 		The path itself
	 */
	public <E extends Bean, G extends GenericProp<E>> EditablePathReferenceBuilder<R, E, P, T> via(PropName<G, E> nextName) {
		return new EditablePathReferenceBuilder<R, E, P, T>(pathRoot, builder.via(nextName));
	}
