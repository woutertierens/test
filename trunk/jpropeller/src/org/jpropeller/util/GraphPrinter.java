...
public class GraphPrinter {
	...
	private void print(Bean bean) {
		...
		if (index > -1) {
			...
			tabIn();
			for (Prop<?> generalProp : bean.features().getList()) {
				
				//If we have single access, just print the single value
				if (generalProp.getAccessType() == PropAccessType.SINGLE) {
					Prop<?> prop = (Prop<?>)generalProp;
					print(generalProp.getName().getString() + " = ");
					tabIn();
					printValue(prop.get());
					tabOut();
				//For list access, print all values in list
				} else if (generalProp.getAccessType() == PropAccessType.LIST) {
					ListProp<?> listProp = (ListProp<?>)generalProp;
					print(generalProp.getName().getString() + " = list of:");
				...
				//This should not happen
					...
					print("Unexpected - unknown access type in General prop - " + generalProp.toString());
				}
			}
			tabOut();
		}
