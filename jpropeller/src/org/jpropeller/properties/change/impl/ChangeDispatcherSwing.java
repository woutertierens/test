...
public class ChangeDispatcherSwing implements ChangeDispatcher {
	...
	/**
	 * Use an {@link IdentityHashMap} as an "IdentitySet" to store the listeners we want to notify
	 */
	private Map<ChangeListener, Object> affectedListeners = new IdentityHashMap<ChangeListener, Object>();
		...
		try {
			...
			for (Changeable changeable : source.changes().keySet()) {
				for (ChangeListener listener : changeable.features().listenerList()) {
					affectedListeners.put(listener, null);
				}
			}

			//Call change on all listeners
			for (ChangeListener listener : affectedListeners.keySet()) {
