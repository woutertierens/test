 ...
 * when multiple regions of the list change, the ListChange
 * would have to represent this change as a larger single region
 * changing).
 * 
 * A ListChange represents only a single _type_ of change - for example
 ...
 * a List<ListChange> can be used, this can represent multi-region,
 * multi-type changes by breaking the change down into multiple operations.
 * Due to the way Lists are used, most changes caused by calling List methods
 * will in fact constitute a single ListChange, the exceptions 
