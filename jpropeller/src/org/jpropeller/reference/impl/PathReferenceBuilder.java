...
public class PathReferenceBuilder<R extends Bean, D extends Bean, P extends GenericProp<T>, T> {
	...
	protected PathReferenceBuilder(PropName<Prop<T>, T> name, R pathRoot, BeanPathBuilder<R, D> builder) {
		super();
		this.name = name;
	...
	protected PathReferenceBuilder(String name, Class<T> clazz, R pathRoot, BeanPathBuilder<R, D> builder) {
		this(PropName.create(name, clazz), pathRoot, builder);
	}

	/**
	 * Start a {@link PathReferenceBuilder} that can be used to build an {@link PathProp}
	...
	public static <R extends Bean, P extends GenericProp<T>, T> PathReferenceBuilder<R, R, P, T> from(PropName<Prop<T>, T> name, R pathRoot) {
		return new PathReferenceBuilder<R, R, P, T>(name, pathRoot, BeanPathBuilder.<R>create());
	}
	
	/**
	 * Start a {@link PathReferenceBuilder} that can be used to build a {@link PathProp}
	 * by use of {@link #via(Transformer)} and {@link #to(Transformer)} methods.
	 * For example, to make a {@link PathProp} starting from bean b and progressing
	 * via properties x, y, z in order, use:
	 * <pre>
	 ...
	 */
	public static <R extends Bean, P extends GenericProp<T>, T> PathReferenceBuilder<R, R, P, T> from(String name, Class<T> clazz, R pathRoot) {
		return new PathReferenceBuilder<R, R, P, T>(name, clazz, pathRoot, BeanPathBuilder.<R>create());
	}
	...
	public <E extends Bean> PathReferenceBuilder<R, E, P, T> via(Transformer<? super D, ? extends GenericProp<E>> nextTransform) {
		return new PathReferenceBuilder<R, E, P, T>(name, pathRoot, builder.via(nextTransform));
	}
	/**
	...
	public <E extends Bean, G extends GenericProp<E>> PathReferenceBuilder<R, E, P, T> via(PropName<G, E> nextName) {
		return new PathReferenceBuilder<R, E, P, T>(name, pathRoot, builder.via(nextName));
	}
